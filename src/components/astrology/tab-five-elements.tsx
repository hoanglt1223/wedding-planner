...
export function TabFiveElements({ brideYear, groomYear, brideName, groomName, brideGender: _brideGender, groomGender: _groomGender }: TabFiveElementsProps) {
    ...
    <div className="space-y-3">
      ...
      <div className="bg-[var(--theme-surface)] rounded-xl shadow-sm border border-[var(--theme-border)] pt-4 pb-3 px-4">
        ...
        <div className="grid grid-cols-5 gap-1 text-center text-xs mb-3">
          {ELEMENT_LIST.map((el) => (
            <div key={el} className={`rounded-lg p-2 ${ELEMENT_BG_CLASS[el]} ${brideSoundElement.element === el || groomSoundElement.element === el ? "ring-2 ring-[var(--theme-primary)]" : ""}`}>
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {ELEMENT_LIST.map((el) => {
                ...
                return (
                  <tr key={el} className="border-b border-[var(--theme-border)]">
...
function ElementDetail({ label, year }: { label: string; year: number }) {
  ...
  return (
    <div className={`bg-[var(--theme-surface)] rounded-xl shadow-sm border border-[var(--theme-border)] pt-3 pb-2 px-4 ${ELEMENT_BG_CLASS[el]}`}>
          ...
          <span className="inline-flex gap-1 ml-1">
            {colors.hex.map((hex, i) => (
              <span key={hex} className="inline-flex items-center gap-0.5">
                <span className="w-3 h-3 rounded-full border border-[var(--theme-border)] inline-block" style={{ background: hex }} />
