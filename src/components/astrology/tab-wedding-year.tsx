...
export function TabWeddingYear({ weddingYear, brideYear, groomYear, brideName, groomName, brideGender: _brideGender, groomGender: _groomGender }: TabWeddingYearProps) {
    ...
    <div className="space-y-3">
      ...
      <div className="bg-[var(--theme-surface)] rounded-xl shadow-sm border border-[var(--theme-border)] pt-4 pb-3 px-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <div className="text-sm text-muted-foreground">{soundElement.name} ({ELEMENT_EMOJI[soundElement.element]} {ELEMENT_LABEL[soundElement.element]})</div>
            <div className="text-sm text-muted-foreground">Năm {zodiac.name} ({zodiac.chi})</div>
          </div>
        </div>
      ...
      <div className="bg-[var(--theme-surface)] rounded-xl shadow-sm border border-[var(--theme-border)] pt-4 pb-3 px-4">
        ...
        <div className="mt-3 text-xs text-muted-foreground">
          <div className="font-medium text-muted-foreground mb-1">Ý nghĩa các vị trí Kim Lâu:</div>
        ...
        <div className="flex flex-wrap gap-2">
          {getGoodYears(brideYear, groomYear, weddingYear).map((y) => (
            <span key={y.year} className={`text-xs px-2 py-1 rounded-full border ${
              y.year === weddingYear ? "bg-[var(--theme-primary-light)] border-[var(--theme-primary)] font-bold" : "bg-green-50 border-green-200"
            }`}>
...
function GoldenLockCard({ label, kl }: { label: string; kl: ReturnType<typeof getGoldenLock> }) {
  return (
    <div className={`rounded-lg p-3 border text-sm ${kl.isKimLau ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"}`}>
      ...
      <div className="text-xs text-muted-foreground mt-1">Tuổi mụ: {kl.lunarAge} (vị trí {kl.pos}/9)</div>
...
function DisasterList({ label, birthYear, weddingYear }: { label: string; birthYear: number; weddingYear: number }) {
  ...
  return (
    <div className="rounded-lg bg-[var(--theme-surface-muted)] p-2 text-xs">
      <div className="font-semibold text-muted-foreground mb-1">{label}</div>
      <div className="space-y-0.5">
        {years.map((y) => (
          <div key={y} className={y === weddingYear ? "text-red-600 font-bold" : "text-muted-foreground"}>
