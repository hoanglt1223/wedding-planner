...
export function TabFengShui({ brideYear, groomYear, brideName, groomName, brideGender = "female", groomGender = "male" }: TabFengShuiProps) {
...
function PalaceCard({ label, year, gender }: { label: string; year: number; gender: "male" | "female" }) {
  ...
  return (
    <div className={`bg-[var(--theme-surface)] rounded-xl shadow-sm border border-[var(--theme-border)] pt-3 pb-2 px-3 text-center ${ELEMENT_BG_CLASS[cung.element]}`}>
      <div className="text-xs text-muted-foreground">{label}</div>
      <div className="text-lg font-bold mt-1">Cung {cung.name}</div>
      <div className="text-sm text-muted-foreground">{ELEMENT_EMOJI[cung.element]} {ELEMENT_LABEL[cung.element]} · {cung.direction}</div>
      <div className="text-xs mt-1 text-muted-foreground">{GROUP_LABEL[cung.group]}</div>
    </div>
  );
}
      ...
      <span className="inline-flex gap-1.5 flex-wrap mt-1">
        {colors.hex.map((hex, i) => (
          <span key={hex} className="inline-flex items-center gap-0.5">
            <span className="w-4 h-4 rounded-full border border-[var(--theme-border)]" style={{ background: hex }} />
            <span>{colors.colors[i]}</span>
          </span>
        ))}
        {sinhColors.hex.map((hex, i) => (
          <span key={`s-${hex}`} className="inline-flex items-center gap-0.5 opacity-70">
            <span className="w-4 h-4 rounded-full border border-dashed border-[var(--theme-border)]" style={{ background: hex }} />
