    ...
    <div className="bg-white rounded-xl p-3 shadow-sm mb-2">
      ...
      <div className="grid grid-cols-3 gap-2 max-sm:grid-cols-2">
        {people.map((person, i) => (
          ...
          >
            <div className="text-2xl mb-1">{person.a}</div>
            <div className="text-xs font-semibold text-gray-800 leading-tight">{person.n}</div>
            {person.r && (
              <div className="text-[0.6rem] text-gray-500 mt-0.5">{person.r}</div>
            )}
          </div>
        ))}
      </div>
