import type { Ceremony, WeddingStep } from "@/types/wedding";
...
export function HandbookPage({
  ...
  const checkable = ceremony.steps.filter((s) => s.checkable);
  const sequence = ceremony.steps.filter((s) => !s.checkable);
  ...
  return (
    ...
    >
      ...
      <div className="px-8 py-6 sm:px-12 sm:py-8">
        {/* Chapter header — only on first page of the step */}
        {isFirst ? (
          ...
          <div className="flex items-center gap-2 mb-4 pb-2 border-b border-gray-200">
            ...
            <span className="text-sm font-semibold text-gray-400 font-serif">
              {step.title}
            </span>
        ...
        {/* Meaning callout — only on first page */}
        {isFirst && step.meaning && (
        ...
        <div className="mb-4">
          <h3 className="text-sm sm:text-base font-bold mb-2 flex items-center gap-2 flex-wrap">
            <span className="text-2xs text-gray-400 font-mono">{ceremonyIndex + 1}.</span>
            ...
            >
              {ceremony.required ? t("BẮT BUỘC", lang) : t("TÙY CHỌN", lang)}
            </span>
          </h3>
          ...
          {/* Gifts */}
          {ceremony.gifts && ceremony.gifts.length > 0 && (
            <div className="mt-2 ml-1 mb-3">
              <div className="text-xs font-semibold text-gray-500 mb-1 uppercase">
                {t("Lễ vật:", lang)}
              </div>
              <div className="space-y-1">
                {ceremony.gifts.map((gift, gi) => (
        ...
        {/* Step notes — only on last page of the step */}
        {isLast && step.notes && step.notes.length > 0 && (
